describe EhjobAuthentication do
  describe '.config' do
    it 'inits Configs obj' do
      EhjobAuthentication.configure do |config|
        config.app = 'EH'
        config.base_url = 'http://www.employmenthero.com'
      end

      config = EhjobAuthentication.config
      expect(config).to be_instance_of(EhjobAuthentication::Config)
      expect(config.app).to eq 'EH'
      expect(config.base_url).to eq 'http://www.employmenthero.com'
      expect(config.authenticate_url).to eq 'http://www.employmenthero.com/api/users/authenticate'
    end
  end
end
